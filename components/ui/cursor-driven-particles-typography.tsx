...
export interface CursorDrivenParticleTypographyProps {
  ...
  returnSpeed?: number;
  color?: string;
  /** Optional second color — when set, particles are tinted left-to-right from `color` to this. */
  gradientTo?: string;
}
...
export function CursorDrivenParticleTypography({
    ...
    const init = () => {
      ...
      for (let y = 0; y < textCoordinates.height; y += step) {
        for (let x = 0; x < textCoordinates.width; x += step) {
          ...
          if (alpha > 128) {
            const particleColor = gradientTo
              ? lerpHex(textColor, gradientTo, x / textCoordinates.width)
              : textColor;
            particles.push(
              new Particle(x / dpr, y / dpr, particleSize, particleColor, dispersionStrength, returnSpeed)
            );
          }
        }
      }
  ...
  }, [text, fontSize, fontFamily, particleSize, particleDensity, dispersionStrength, returnSpeed, color, gradientTo]);
