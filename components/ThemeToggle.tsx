...
export default function ThemeToggle({
  ...
  return (
    <button
      ...
      title={`Switch to ${theme === "dark" ? "light" : "dark"} mode`}
      aria-label={`Switch to ${theme === "dark" ? "light" : "dark"} mode`}
      className={`group relative inline-flex items-center gap-2 rounded-full border border-line bg-surface/60 backdrop-blur-sm px-3.5 py-1.5 font-mono text-[0.65rem] tracking-widest2 text-paper transition-all hover:border-lime hover:text-lime ${className}`}
    >
      <span
        className="inline-block transition-transform duration-300 group-hover:rotate-45 text-xs"
        aria-hidden="true"
      >
        {theme === "dark" ? "☀️" : "🌙"}
      </span>
